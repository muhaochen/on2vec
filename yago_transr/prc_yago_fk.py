...
import os
if not os.path.exists('../../results/yago/prc'):
    os.makedirs('../../results/yago/prc')
...
transr_file = 'yago-transr.ckpt'
data_file = 'yago-data.bin'
test_data = '../../../onto2vec-dataset/Yago/yagoFactsFiltCDTTest.tsv'
result_file = '../../results/yago/prc/prc_transr_tail.txt'
result_file2 = '../../results/yago/prc/prc_transr_head.txt'
result_file3 = '../../results/yago/prc/prc_transr_rel.txt'
...
tester = Tester()
tester.build(save_path = transr_file, data_save_path = data_file, pre_pool=True)
...
def test2(tester, index, score_rel, num_rel):
    while index.value < len(tester.test_triples):
        ...
        # test_rel 
        # [[category, rank]*]
        this_score_rel = np.zeros((len(tester.this_data.rels), 2))
        for rel_pred in range(len(tester.this_data.rels)):
            ...
            if rel_pred in tester.ht_map[h][t]:
                this_score_rel[rel_pred][0] = 1.
                num_rel.value += 1
            this_score_rel[rel_pred][1] = tester.dissimilarity(h, rel_pred, t)
