...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("graph", initializer=orthogonal_initializer()):
            ...
            # Type A loss : [|| (h - M_hr^T h M_hr) + r - ( t - M_tr^T t M_tr )||_2 + m1 - || (h' - M_hr^T h' M_hr) + r - ( t' - M_tr^T t' M_tr ) ||_2]+    here [.]+ means max (. , 0)
            ...
            #A_h_batch_mul = tf.sub(A_h_con_batch, tf.squeeze(tf.batch_matmul(tf.batch_matmul(tf.reshape(A_mat_h_batch, [-1, self.dim, 1]) , tf.expand_dims(A_h_con_batch, 1)), tf.reshape(A_mat_h_batch, [-1, 1, self.dim])), [1]))
            A_h_batch_mul = tf.sub(A_h_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_h_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            # This is a batch of t * M_hr given a batch of (h, r, t)
            #A_t_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_t_con_batch, 1), tf.reshape(A_mat_h_batch, [-1, self.dim, self.dim])), [1])
            # negative sampled h and t
            A_t_batch_mul = tf.sub(A_t_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_t_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            #A_hn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_hn_con_batch, 1), tf.reshape(A_mat_h_batch, [-1, self.dim, self.dim])), [1])
            A_hn_batch_mul = tf.sub(A_hn_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_hn_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            #A_tn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_tn_con_batch, 1), tf.reshape(A_mat_h_batch, [-1, self.dim, self.dim])), [1])
            A_tn_batch_mul = tf.sub(A_tn_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_tn_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            ...
            # [||h|| - 1]+  +  [||t|| - 1]+  +  [||h'|| - 1]+  +  [||t'|| - 1]+
            #A_vec_restraint = tf.concat(0, [tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_h_con_batch), 1)), 1.), 0.), tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_t_con_batch), 1)), 1.), 0.),             tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_hn_con_batch), 1)), 1.), 0.), tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_tn_con_batch), 1)), 1.), 0.)])
            ...
            #A_proj_restraint = tf.concat(0, [tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_h_batch_mul), 1)), 1.), 0.), tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_t_batch_mul), 1)), 1.), 0.), tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_hn_batch_mul), 1)), 1.), 0.), tf.maximum(tf.sub(tf.sqrt(tf.reduce_sum(tf.square(A_tn_batch_mul), 1)), 1.), 0.)])
            ...
            B_t_batch_mul_head = tf.sub(B_con_h_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_h_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            #B_t_batch_mul_tail = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_t_batch, 1), tf.reshape(B_mat_h_batch, [-1, self.dim, self.dim])), [1])
            B_t_batch_mul_tail = tf.sub(B_con_t_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_t_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            ...
            B_tn_batch_mul_head = tf.sub(B_con_hn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_hn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            #B_tn_batch_mul_tail = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 1), tf.reshape(B_mat_h_batch, [-1, self.dim, self.dim])), [1])
            B_tn_batch_mul_tail = tf.sub(B_con_tn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            # t*M_hr + r ~ t*M_tr
            ...
            # multiplication of a batch of vectors and a batch of matrices
            B_h_batch_mul_head = tf.sub(B_con_h_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_h_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            B_h_batch_mul_tail = tf.sub(B_con_t_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_t_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            # multiplication of a batch of vectors and a batch of matrices for negative samples
            B_hn_batch_mul_head = tf.sub(B_con_hn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_hn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            B_hn_batch_mul_tail = tf.sub(B_con_tn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            # t*M_tr - r ~ h*M_hr
