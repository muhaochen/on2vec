...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("graph", initializer=orthogonal_initializer()):
            ...
            #A_h_batch_mul = tf.subtract(A_h_con_batch, tf.squeeze(tf.batch_matmul(tf.batch_matmul(tf.reshape(A_mat_h_batch, [-1, self.dim, 1]) , tf.expand_dims(A_h_con_batch, 1)), tf.reshape(A_mat_h_batch, [-1, 1, self.dim])), [1]))
            A_h_batch_mul = tf.subtract(A_h_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_h_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            ...
            A_t_batch_mul = tf.subtract(A_t_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_t_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            #A_hn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_hn_con_batch, 1), tf.reshape(A_mat_h_batch, [-1, self.dim, self.dim])), [1])
            A_hn_batch_mul = tf.subtract(A_hn_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_hn_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            #A_tn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_tn_con_batch, 1), tf.reshape(A_mat_h_batch, [-1, self.dim, self.dim])), [1])
            A_tn_batch_mul = tf.subtract(A_tn_con_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(A_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(A_tn_con_batch, 2), tf.expand_dims(A_mat_h_batch, 1))), [1]))
            ...
            A_vec_restraint = tf.concat([tf.maximum(tf.subtract(tf.sqrt(tf.reduce_sum(tf.square(A_h_con_batch), 1)), 1.), 0.), tf.maximum(tf.subtract(tf.sqrt(tf.reduce_sum(tf.square(A_t_con_batch), 1)), 1.), 0.)], 0)
            ...
            A_proj_restraint = tf.concat([tf.maximum(tf.subtract(tf.sqrt(tf.reduce_sum(tf.square(A_h_batch_mul), 1)), 1.), 0.), tf.maximum(tf.subtract(tf.sqrt(tf.reduce_sum(tf.square(A_t_batch_mul), 1)), 1.), 0.)], 0)
            ...
            B_t_batch_mul_head = tf.subtract(B_con_h_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_h_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            ...
            B_t_batch_mul_tail = tf.subtract(B_con_t_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_t_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            ...
            B_tn_batch_mul_head = tf.subtract(B_con_hn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_hn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            ...
            B_tn_batch_mul_tail = tf.subtract(B_con_tn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            # t*M_hr + r ~ t*M_tr
            ...
            # multiplication of a batch of vectors and a batch of matrices
            B_h_batch_mul_head = tf.subtract(B_con_h_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_h_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            B_h_batch_mul_tail = tf.subtract(B_con_t_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_t_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            # multiplication of a batch of vectors and a batch of matrices for negative samples
            B_hn_batch_mul_head = tf.subtract(B_con_hn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_hn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            B_hn_batch_mul_tail = tf.subtract(B_con_tn_batch, tf.squeeze(tf.batch_matmul(tf.expand_dims(B_mat_h_batch, 1), tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 2), tf.expand_dims(B_mat_h_batch, 1))), [1]))
            # t*M_tr - r ~ h*M_hr
