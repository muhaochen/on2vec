...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("graph", initializer=orthogonal_initializer()):
            ...
            B_h_batch_mul_tail = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_t_batch, 1), tf.reshape(B_mat_h_batch, [-1, self.dim, self.dim])), [1])
            # multiplication of a batch of vectors and a batch of matrices for negative samples
            ...
            B_hn_batch_mul_tail = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 1), tf.reshape(B_mat_h_batch, [-1, self.dim, self.dim])), [1])
            # t*M_tr - r ~ h*M_hr
