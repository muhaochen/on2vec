...
class Trainer(object):
    def __init__(self):
        ...
        self.this_data = None
        self.tf_parts = None
        self.save_path = 'this-transd.ckpt'
        self.data_save_path = 'this-data.bin'
        self.L1=False

    def build(self, data, dim=64, batch_size=128, save_path = 'this-transd.ckpt', data_save_path = 'this-data.bin', L1=False):
        self.this_data = data
    ...
    def train(self, epochs=20, save_every_epoch=10, lr=0.001, a1=0.1, a2=0.05, m1=0.5, m2=0.5, balance_alpha1=False, 
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % save_every_epoch == 0:
                ...
                print("transd saved in file: %s. Data saved in file: %s" % (this_save_path, self.data_save_path))
        this_save_path = self.tf_parts._saver.save(sess, self.save_path)
        print("transd saved in file: %s" % this_save_path)
        sess.close()
        print("Done")
...
def load_tfparts(batch_size = 128,
                dim = 64,
                this_data=None,
                save_path = 'this-transd.ckpt', L1=False):
    tf_parts = transd.TFParts(num_rels=this_data.num_rels(),
