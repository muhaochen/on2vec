...
class Tester(object):
    ...
    def build(self, save_path = 'this-model.ckpt', data_save_path = 'this-data.bin', pre_pool=False, use_balltree=False, use_lsh=False):
        ...
        self.tf_parts = transd.TFParts(num_rels=self.this_data.num_rels(),
        ...
        value_ht, value_r, value_cp, value_rp = sess.run([self.tf_parts._ht, self.tf_parts._r, self.tf_parts._htp, self.tf_parts._rp])  # extract values.
        sess.close()
        self.vec_c = np.array(value_ht)
        self.vec_r = np.array(value_r)
        self.cp = np.array(value_cp)
        self.rp = np.array(value_rp)
        #self.ids = np.identity(self.this_data.dim)
        self.pj_pool = {}
        ...
        if pre_pool:
            ...
            for r in range(len(self.vec_r)):
                new_pool = self.projection_pool(self.vec_c, r)
                self.pj_pool[r] = new_pool
        '''
        self.balltree=None
        self.lsh=None
        if use_balltree:
    ...
    def projection(self, c, r, h_or_t='h'):
        mat = None
        vec_c = self.vec_c[c]
        cp = self.cp[c].reshape((self.this_data.dim, 1))
        rp = self.rp[r].reshape((1, self.this_data.dim))
        #return np.dot(vec_c, np.dot(cp, rp) + self.ids)
        return np.dot(vec_c, np.dot(cp, rp))
    '''
    ...
    def projection_pool(self, ht, r, h_or_t='h'):
        ...
        rst = np.zeros((len(ht), self.this_data.dim))
        for i in range(len(rst)):
            #rst[i] = np.dot(ht[i], np.dot(self.cp[i], self.rp[r]) + self.ids)
            rst[i] = np.dot(ht[i], np.dot(self.cp[i].reshape((self.this_data.dim, 1)), self.rp[r].reshape((1, self.this_data.dim))))
        if self.pre_pool and self.pj_pool.get(r) == None:
            self.pj_pool[r] = rst
        return rst
