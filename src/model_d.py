...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("graph", initializer=orthogonal_initializer()):
			...
			D_h_batch_mul = tf.squeeze(tf.matmul(tf.expand_dims(D_con_h_batch, 1), tf.reshape(D_mat_h_batch, [-1, self.dim, self.dim])), [1])
            D_t_batch_mul = tf.squeeze(tf.matmul(tf.expand_dims(D_con_t_batch, 1), tf.reshape(D_mat_t_batch, [-1, self.dim, self.dim])), [1])
			D_hn_batch_mul = tf.squeeze(tf.matmul(tf.expand_dims(D_con_hn_batch, 1), tf.reshape(D_mat_h_batch, [-1, self.dim, self.dim])), [1])
            D_tn_batch_mul = tf.squeeze(tf.matmul(tf.expand_dims(D_con_tn_batch, 1), tf.reshape(D_mat_t_batch, [-1, self.dim, self.dim])), [1])
