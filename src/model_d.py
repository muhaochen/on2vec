    ...
    def build(self):
        ...
        with tf.variable_scope("graph", initializer=orthogonal_initializer()):
            ...
            self._D_h_index = D_h_index = tf.placeholder(
                ...
                shape=[self.batch_size],
                name='D_h_index')
            self._D_hn_index = D_hn_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self.batch_size],
                name='D_hn_index')
            self._D_r_index = D_r_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self.batch_size],
                name='D_r_index')
            self._D_t_index = D_t_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self.batch_size],
                name='D_t_index')
            self._D_tn_index = D_tn_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self.batch_size],
                name='D_tn_index')

			D_con_h_batch = tf.nn.embedding_lookup(ht,D_h_index)
            D_con_t_batch = tf.nn.embedding_lookup(ht,D_t_index)
			D_con_hn_batch = tf.nn.embedding_lookup(ht,D_hn_index)
            D_con_tn_batch = tf.nn.embedding_lookup(ht,D_tn_index)
            D_mat_h_batch = tf.nn.embedding_lookup(Mh,D_r_index)
            D_mat_t_batch = tf.nn.embedding_lookup(Mt,D_r_index)
            
			D_h_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(D_con_h_batch, 1), tf.reshape(D_mat_h_batch, [-1, self.dim, self.dim])), [1])
            D_t_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(D_con_t_batch, 1), tf.reshape(D_mat_t_batch, [-1, self.dim, self.dim])), [1])
			D_hn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(D_con_hn_batch, 1), tf.reshape(D_mat_h_batch, [-1, self.dim, self.dim])), [1])
            D_tn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(D_con_tn_batch, 1), tf.reshape(D_mat_t_batch, [-1, self.dim, self.dim])), [1])
