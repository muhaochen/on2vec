...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("graph", initializer=orthogonal_initializer()):
            ...
            #self._ids = ids = np.repeat(np.identity(self.dim).reshape((1, -1)), self._batch_size).reshape((-1, self.dim, self.dim))
            ...
            self._r = r = tf.get_variable(
                ...
                dtype=tf.float32)
            self._htp = htp = tf.get_variable(
                name='htp',  # for t AND h
                shape=[self.num_cons, self.dim],
                dtype=tf.float32)
            self._rp = rp = tf.get_variable(
                name='rp',
                shape=[self.num_rels, self.dim],
                dtype=tf.float32)
            ...
            A_sc_h_batch = tf.nn.embedding_lookup(htp, A_h_index)
            A_sc_t_batch = tf.nn.embedding_lookup(htp, A_t_index)
            A_sc_r_batch = tf.nn.embedding_lookup(rp, A_r_index)
            A_mat_h_batch = tf.batch_matmul(tf.expand_dims(A_sc_h_batch, 2), tf.expand_dims(A_sc_r_batch, 1))
            A_mat_t_batch = tf.batch_matmul(tf.expand_dims(A_sc_t_batch, 2), tf.expand_dims(A_sc_r_batch, 1))
            A_hn_con_batch = tf.nn.embedding_lookup(ht,A_hn_index)
            A_tn_con_batch = tf.nn.embedding_lookup(ht,A_tn_index)
            A_sc_hn_batch = tf.nn.embedding_lookup(htp, A_hn_index)
            A_sc_tn_batch = tf.nn.embedding_lookup(htp, A_tn_index)
            A_mat_hn_batch = tf.batch_matmul(tf.expand_dims(A_sc_hn_batch, 2), tf.expand_dims(A_sc_r_batch, 1))
            A_mat_tn_batch = tf.batch_matmul(tf.expand_dims(A_sc_tn_batch, 2), tf.expand_dims(A_sc_r_batch, 1))
            ...
            A_hn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_hn_con_batch, 1), tf.reshape(A_mat_hn_batch, [-1, self.dim, self.dim])), [1])
            A_tn_batch_mul = tf.squeeze(tf.batch_matmul(tf.expand_dims(A_tn_con_batch, 1), tf.reshape(A_mat_tn_batch, [-1, self.dim, self.dim])), [1])
            ...
            B_sc_h_batch = tf.nn.embedding_lookup(htp, B_h_index)
            B_sc_t_batch = tf.nn.embedding_lookup(htp, B_t_index)
            B_sc_r_batch = tf.nn.embedding_lookup(rp, B_r_index)
            B_mat_h_batch = tf.batch_matmul(tf.expand_dims(B_sc_h_batch, 2), tf.expand_dims(B_sc_r_batch, 1))
            B_mat_t_batch = tf.batch_matmul(tf.expand_dims(B_sc_t_batch, 2), tf.expand_dims(B_sc_r_batch, 1))
            B_hn_con_batch = tf.nn.embedding_lookup(ht,B_hn_index)
            B_tn_con_batch = tf.nn.embedding_lookup(ht,B_tn_index)
            B_sc_hn_batch = tf.nn.embedding_lookup(htp, B_hn_index)
            B_sc_tn_batch = tf.nn.embedding_lookup(htp, B_tn_index)
            B_mat_hn_batch = tf.batch_matmul(tf.expand_dims(B_sc_hn_batch, 2), tf.expand_dims(B_sc_r_batch, 1))
            B_mat_tn_batch = tf.batch_matmul(tf.expand_dims(B_sc_tn_batch, 2), tf.expand_dims(B_sc_r_batch, 1))
            ...
            B_tn_batch_mul_head = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_hn_batch, 1), tf.reshape(B_mat_hn_batch, [-1, self.dim, self.dim])), [1])
            B_tn_batch_mul_tail = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 1), tf.reshape(B_mat_tn_batch, [-1, self.dim, self.dim])), [1])
            # t*M_hr + r ~ t*M_tr
            ...
            # multiplication of a batch of vectors and a batch of matrices for negative samples
            B_hn_batch_mul_head = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_hn_batch, 1), tf.reshape(B_mat_hn_batch, [-1, self.dim, self.dim])), [1])
            B_hn_batch_mul_tail = tf.squeeze(tf.batch_matmul(tf.expand_dims(B_con_tn_batch, 1), tf.reshape(B_mat_tn_batch, [-1, self.dim, self.dim])), [1])
            # t*M_tr - r ~ h*M_hr
